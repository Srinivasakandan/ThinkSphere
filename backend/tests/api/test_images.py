...
def _create_inspection(client) -> str:
    response = client.post("/api/inspections", json={"product_name": "Test Product"})
...
def test_rejects_non_image_file(client):
    ...
    response = client.post(f"/api/inspections/{inspection_id}/images", files=files)
...
def test_upload_and_delete_image(client):
    ...
    upload_response = client.post(
        f"/api/inspections/{inspection_id}/images", files=files, data={"view_types": ["FRONT"]}
    )
    assert upload_response.status_code == 200
    image_id = upload_response.json()[0]["id"]

    delete_response = client.delete(f"/api/inspections/{inspection_id}/images/{image_id}")
...
def test_delete_nonexistent_image_returns_404(client):
    inspection_id = _create_inspection(client)
    response = client.delete(f"/api/inspections/{inspection_id}/images/00000000-0000-0000-0000-000000000099")
