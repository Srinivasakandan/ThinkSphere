...
def _upload_four_images(client, inspection_id: str) -> list[dict]:
    ...
    response = client.post(f"/api/v1/inspections/{inspection_id}/images", files=files, data=data)
...
def test_full_inspection_acceptance_flow(client, seeded_rules):
    ...
    process_response = client.post(f"/api/v1/inspections/{inspection_id}/process")
    ...
    status_response = client.get(f"/api/v1/inspections/{inspection_id}/processing-status")
    ...
    extracted = client.get(f"/api/v1/inspections/{inspection_id}/extracted").json()["fields"]
    assert len(extracted) > 0
    for field in extracted:
        assert field["confidence_level"] in ("HIGH", "MEDIUM", "LOW")

    # 14-17: applicable demo rules were selected and evaluated.
    rules_response = client.get(f"/api/v1/inspections/{inspection_id}/rules")
    ...
    results = client.get(f"/api/v1/inspections/{inspection_id}/results").json()
    ...
    if non_passing:
        ...
        review_response = client.patch(
            f"/api/v1/inspections/{inspection_id}/rules/{target['id']}/review",
        ...
        remaining = [
            r
            for r in client.get(f"/api/v1/inspections/{inspection_id}/rules").json()["rules"]
            if r["status"] != "PASS" and not r["reviewed"]
        ]
        for extra in remaining:
            client.patch(
                f"/api/v1/inspections/{inspection_id}/rules/{extra['id']}/review",
                json={"reviewed": True, "inspector_note": "Reviewed."},
            )

    # 24-26: finalize; the backend must record inspector, timestamp, final status.
    finalize_response = client.post(f"/api/v1/inspections/{inspection_id}/finalize", json={})
    ...
    recent = client.get("/api/v1/dashboard/recent-inspections").json()["items"]
...
def test_finalize_is_blocked_while_findings_are_unreviewed(client, seeded_rules):
    ...
    rule_results = client.get(f"/api/v1/inspections/{inspection_id}/rules").json()["rules"]
...
def test_cannot_process_inspection_with_no_images(client, seeded_rules):
    inspection_id = _create_inspection(client)
    response = client.post(f"/api/v1/inspections/{inspection_id}/process")
...
def test_field_correction_marks_manually_verified(client, seeded_rules):
    ...
    response = client.patch(
        f"/api/v1/inspections/{inspection_id}/fields/{field['id']}",
        json={
